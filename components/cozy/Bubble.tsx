...
export function Bubble({ msg, onOpenImage, agentName = 'CozyAI', agentAvatar }: Props) {
  if (msg.role === 'system') {
      ...
      <div className="flex flex-col self-end items-end max-w-[86%] gap-1.5">
        ...
        {msg.content && (
          <div className="bg-surface-bubble text-text-1 rounded-[18px_18px_4px_18px] px-4 py-3 max-w-full">
            <div className="line-clamp-5 whitespace-pre-wrap break-words leading-6 max-h-[120px]">
              {msg.content}
            </div>
          </div>
        )}
  ...
  return (
    <div className="flex flex-col self-start items-start max-w-[86%] gap-1.5">
      <div className="flex items-center gap-1.5 mb-1.5">
        ...
        <span className="text-xs font-semibold text-brand-rose-500 opacity-75">{name}</span>
      </div>
      <div
        className={cn(
          'text-text-1 px-0.5',
          'font-aeonik text-[18px] leading-[140%] whitespace-pre-wrap break-words'
        )}
      >
        {msg.content}
      </div>
      {msg.reference && (
        <div className="mt-2.5 text-[14px] leading-5 tracking-[0.28px] text-text-2 font-aeonik">
          {msg.reference}
        </div>
      )}
    </div>
...
function SystemMessage({ content }: { content: string }) {
  ...
  return (
    <div className="cozy-tier-3 self-center max-w-[90%] px-3 py-0.5 my-1">{content}</div>
  );
}
